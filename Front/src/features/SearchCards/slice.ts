...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    searchCardsPush(state: SearchCards, action: PayloadAction<Card[]>): void {
      ...
      state.cardsFound = state.cardsFound.filter(
        ({ name: refName }, pos, arr) => arr.map(({ name }) => name).indexOf(refName) === pos,
      );
    },
...
export const searchCards =
  ...
  async (dispatch: AppDispatch, getState: () => RootState): Promise<void> => {
    try {
      ...
      let nextRequest = `https://api.magicthegathering.io/v1/cards?name=${encodeURI(name)}`;
      let hasNext = true;
      let isFirst = true;
      do {
